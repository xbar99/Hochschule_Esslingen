...
const minesweeper = {
    ...
    async startNewGame(_gameType) {
        for(let i = 0; i < this.gameTypes.length; i++) {
            if(_gameType === this.gameTypes[i].name) {
                ...
                await this.logic.init(this.size, this.numberOfMines); 
            }
        }
    },
    toggleFlag(divCell) {
    ...
    async cellClicked(event) {
        ...
        const mineStatus = await this.logic.sweep(x, y);
    
        if (!this.mines) {
            this.mines = mineStatus.mines;
            console.dir(this.mines);
        }
    
        if(mineStatus.minehit) {
            this.placeSymbol(x, y, 'cellsymbolm');
            this.showMines(this.mines, false);
        ...
        if (mineStatus.userwins) {
            this.showMines(this.mines, true);
        }
    
        event.preventDefault();
    },    
    showMines(mines, userWins) {
        if (Array.isArray(mines)) {
            for (const mine of mines) {
                const divMines = document.querySelector(`div[data-x="${mine.x}"][data-y="${mine.y}"]`);
                if (divMines) {
        ...
        this.gameEndScreen(userWins);
    },    
    gameEndScreen(userWins) {
...
const localLogic = {
    ...
    async init(_size, _numberOfMines) {
        this.uncoveredCells = [];
        this.moveCounter = 0;
    ...
    initializeMines(x, y) {
        ...
        while (minesAdded < this.numberOfMines) {
            ...
            if(!(this.field[randomRow][randomCol]) && (randomRow !== y) && (randomCol !== x)) {
                this.field[randomRow][randomCol] = true;
                minesAdded++;
            }
        }
    },
    ...
    updateUncoveredCells(emptyCells = undefined, x, y) {
    ...
    async sweep(x, y) {
        ...
        if (this.field[y][x] === true) {
            ...
            return { minehit: true, mines: _mines }; 
        } else {
            ...
            const _userwins = this.nahIdWin();
            return { minehit: false, minesAround: numberOfMinesAround, emptyCells: _emptyCells, userwins: _userwins, mines: _mines };
        }
    },
    countNeighbours(x,y) {
        let count = 0;
...
const remoteLogic = {
    ...
    async fetchAndDecode(request) {
        return fetch(`${this.server}${request}`).then(response => response.json());
    },
    async init(_size, _numberOfMines) { 
        const request = `request=init&size=${_size}&mines=${_numberOfMines}&userid=${this.userid}`;
        const response = await this.fetchAndDecode(request);

        console.log(response); //hier, kann weg!
